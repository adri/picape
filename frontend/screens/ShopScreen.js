...
export default function ShopScreen({ navigation }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScrollView style={{ flex: 1, paddingBottom: 70 }} stickyHeaderIndices={[0]}>
        <CloseIcon
              ...
              <View>
                ...
                {ingredients.map((ingredient) => (
                  <ListItem
                    ...
                    textStyle={{
                      textDecorationLine: ingredient.isBought ? 'line-through' : null,
                    }}
                    key={ingredient.id}
                    title={ingredient.name}
                    imageUrl={ingredient.imageUrl}>
                    <Badge amount={ingredient.orderedQuantity} />
                    <BuyIngredient id={ingredient.id} isBought={ingredient.isBought} />
                  </ListItem>
                ))}
              </View>
            );
