...
- **Seasonal** (`seasonal.ex`) — a hand-written Dutch growing calendar and the recipe ranking the `seasonal_produce` MCP tool answers with. Pure data, no supervision, no HTTP. It carries only produce whose Dutch season changes through the year, so year-round staples and imports are absent on purpose and an ingredient it does not name is unknown, not out of season. Do not source it from the supermarket's `provenanceStatement`: that is a calendar of origin, it calls greenhouse strawberries Dutch in all twelve months, and it is empty for pumpkin, stewing pears, leek, spinach and green beans.
...
- `edit_ingredient` is a partial update: it sends the contexts only the fields the caller passed, so an agent can move a delisted ingredient to another supermarket product without resending its name. `Ingredients.edit_ingredient/1` refetches `supermarket_product_raw` only when the params carry a `supermarket_product_id`, and only replaces the tags when they carry `tag_ids`.
- `recipe_history` and `ingredient_history` read `Order.recipe_history/2` and `Order.ingredient_history/2`, which count finished orders the way `previously_ordered_ingredients/1` does: every `line_id` but the live cart `"1"`, including the supermarket's own order numbers from before it minted its own ids, and no parsing of that id. A `quantity` of 0 is an ingredient taken off a list, not a buy, and `unplanned` is a recipe that was dropped, so neither counts. `ingredient_history` returns `times_bought_last_year` beside the mean gap between buys because a gap alone cannot tell an ingredient that is due from one that was dropped; don't drop that field to make the reply smaller. `mark_recipe_as_cooked` writes to `Order.last_order_id()`, the same order the `markRecipeAsCooked` mutation writes to.
...
- `npm run build:web` runs `expo export --platform web` into `frontend/dist`, then `node scripts/build-launch-images.js` and `node scripts/build-service-worker.js`. The first writes the iOS launch screens into `dist/pwa/apple-touch-startup-image/` and injects their `<link>` tags into `dist/index.html`; iOS matches them on exact device pixels with no fallback, so a new iPhone screen size needs a line in that script's `SIZES` or that phone gets a white flash on launch. They are flat colour, one per theme, from the `background` tokens in [constants/Colors.js](frontend/constants/Colors.js). `build-service-worker.js` calls workbox-build's `generateSW` in place of the old `workbox-webpack-plugin` `InjectManifest`, which only worked inside webpack. It must emit one self-contained classic worker (`inlineWorkboxRuntime: true`); the default split runtime needs `{type: 'module'}` registration, which the app does not use.
...
- **`animationKeyframes` only works inside `StyleSheet.create`.** react-native-web builds an `@keyframes` rule from it in its atomic compiler; its inline path drops the property and says so in a comment. An inline `animationKeyframes` therefore renders as an `animation-duration` against `animation-name: none`, which animates nothing and warns about nothing. Three places in this app had it and none of them ever animated. Check `getComputedStyle(el).animationName` in the browser before believing a keyframe animation runs. Reanimated is not the answer: it was researched and rejected, because on web it runs a main-thread rAF loop for ~150 KB gzipped.
- Motion tokens and the `prefers-reduced-motion` read live in [constants/Motion.js](frontend/constants/Motion.js). `StyleSheet.create` compiles no `@media` rule, so the preference has to be read in JavaScript; `AccessibilityInfo.isReduceMotionEnabled` wraps the same query but is promise-based and answers `true` where there is no DOM. In Playwright, set it with `page.emulateMedia({ reducedMotion: 'reduce' })` before the first navigation: `test.use({ reducedMotion: 'reduce' })` resolves the fixture but never reaches the context, and the page still reads no-preference.
...
- `priv/repo/seeds.exs` is the e2e dataset. `bin/phx --fake` loads it into `picape_e2e`, and the Playwright screenshots depend on it. The plan screen groups recipes by ingredient names such as `rijst`, `pasta` and `wrap`, so keep those names Dutch. It ends with `Picape.Ingredients.match_supermarket_products/0`, the nightly job that fills `supermarket_product_raw`: without it a seeded ingredient has no picture, no price, no nutriscore and no description, and every screen that reads the product card is blank. The call goes to whatever `SUPERMARKET_BASE_URL` the seed run points at, so `bin/phx --fake` fills it from the fixtures, and it rescues its own failures so a seed without a supermarket still finishes.
